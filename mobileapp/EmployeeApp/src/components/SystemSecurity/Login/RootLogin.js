...
export default class RootLogIn extends ValidationComponent {
  ...
  login = () => {
    ...
    var loginValues = { email: email, password: password, role:'employee' }
    if (this.validInput()) {
      ...
      axios.post('/api/auth/post/login', loginValues)
        .then(response => {
          ...
          if (response.data.message === "INVALID_PASSWORD" || response.data.message === "NOT_REGISTER" || response.data.message === "USER_UNVERIFIED") {
            ...
            console.log("Inside")
            this.props.navigation('Tripbooking')
          }
        })
        .catch(error => {
  ...
  render() {
    ...
    return (
          ...
          <View style={{}}>
            <View style={styles.textTitleWrapper}><Text style={styles.signintxt}>Sign In</Text></View>
