...
class CompletedBookings extends ValidationComponent {
    constructor(props) {
        super(props);
        this.state = {
            imgarr: [
            ...
            <Card key={index} containerStyle={styles.sliderView}>
                <View style={{flexDirection:'row'}}>
                     ...
                     <View style={styles.carsbookings}>
                        ...
                        <View style={{alignSelf:'flex-end',backgroundColor:"#5DC853",paddingHorizontal:10,paddingVertical:2,borderRadius:100}}>
                            <Text style={styles.itemtitlename,{color:"#fff"}}>{item.status[item.status.length-1].value}</Text>
                        </View>     
                    </View>
                    <TouchableOpacity style={[styles.viewBookingTickets,{backgroundColor:"#5DC853"}]} onPress={()=>this.props.navigation.navigate('BookingsPageView')} >
...
export default withNavigation(CompletedBookings);
