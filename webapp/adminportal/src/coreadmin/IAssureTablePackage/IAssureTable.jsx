			...
			<div id="tableComponent" className="col-lg-12 col-sm-12 col-md-12 col-xs-12">
				...
				{this.state.tableObjects.downloadApply === true ?
					this.state.tableData && this.state.id && this.state.tableName && this.state.tableData.length !== 0 ?
						<React.Fragment>
							<div className="col-lg-1 col-md-1 col-xs-12 col-sm-12 NOpadding  pull-right " title="Download as Excel">
									...
									{this.state.tableHeading ?
										Object.entries(this.state.tableHeading).map(
											([key, value], i) => {
												...
												if (key === 'empName') { 
													return (
														<th key={i} className="umDynamicHeader srpadd text-center"style={{width:"140px"}}>{value}</th>
													);
												}  
												if (key === 'contactDetails') { 
													return (
														<th key={i} className="umDynamicHeader srpadd text-center"style={{width:"150px"}}>{value}</th>
													);
												}  
												if (key === 'workDetails') { 
													return (
														<th key={i} className="umDynamicHeader srpadd text-center"style={{width:"175px"}}>{value}</th>
													);
												} 
												...
												if (key === 'approvingAuthorityId2') { 
													return (
														<th key={i} className="umDynamicHeader srpadd text-center"style={{width:"138px"}}>{value}</th>
													);
												}  
												else {
													return (
												...
												if(key === 'actions'){
														return(
															<th key={i} className="umDynamicHeader srpadd text-center">
										...
										(value, i) => {
											return (
												<tr key={i} className="">
													...
													:
													<td className="textAlignCenter" style={{width:"35px"}}>{this.state.startRange + 1 + i}</td>
													}
													{
													...
													{this.state.tableHeading && this.state.tableHeading.actions ?
														<td className="textAlignCenter" style={{width:"100px"}} id="ActionContent">
															<span>
								...
								{this.state.tableData && this.state.tableData.length > 0 ?
									this.state.tableData.map(
										(value, i) => {
											return (
												<tr key={i} className="">
													<td className="textAlignCenter fixedWidth_new3">{this.state.startRange + 1 + i}</td>
													{
														Object.entries(value).map(
															([key, value1], i) => {
																if ($.type(value1) === 'string') {
																	...
																	var aN = value2.replace(this.state.reA, "");
																	if (aN && $.type(aN) === 'string') {
																		var textAlign = 'textAlignLeft fixedWidth_new3'
																	} else {
																		var bN = value1 ? parseInt(value1.replace(this.state.reN, ""), 10) : '';
																		if (bN) {
																			var textAlign = 'textAlignLe fixedWidth_new3';
																		} else {
																			var textAlign = 'textAlignLeft fixedWidth_new3';
																		}
																	}
