			...
			<div id="tableComponent" className="col-lg-12 col-sm-12 col-md-12 col-xs-12">
				...
				<div className="col-lg-12 col-sm-12 col-md-12 col-xs-12 NOpadding marginTop8">
				 <table className="table iAssureITtable-bordered table-striped table-hover fixedTable"style={{marginBottom:"0px"}}>
								...
								<tr className="">
									{
										...
										<th className="umDynamicHeader  srpadd text-center"style={{height:"40px"}}>Company ID</th>

										:
										<th className="umDynamicHeader srpadd text-center new_tableborder"style={{height:"40px"},{width:"80px"}}>Sr. No.</th>
					...
					<div className="table-responsive scrollToTable" id="section-to-print">
									...
									this.state.tableData.map(
										(value, i) => {
											return (
												<tr key={i} className="">
													{ this.state.showCompanyId?
													<td className="textAlignLeft fixedWidth_new">{value.companyID}</td>

													:
													<td className="textAlignLeft fixedWidth_new">{this.state.startRange + 1 + i}</td>
													}
													{
															...
															([key, value1], j) => {
																if ($.type(value1) === 'string') {
																	...
																	if (aN && $.type(aN) === 'string') {
																		var textAlign = 'textAlignLeft fixedWidth_new'
																	} else {
																...
																if (found.length > 0) {
																	if (key !== 'id') {
																		...
																		if(value1){
																			return(<td className={textAlign} key={j}><div className={textAlign+" col"+(j+1)+" "+j } dangerouslySetInnerHTML={{ __html:value1}}></div></td>); 						
																		}else{
																			return(<td className={textAlign} key={j}><div className={textAlign+" col"+(j+1)+ " "+j } dangerouslySetInnerHTML={{ __html:value1}}></div></td>); 						
																		}
																	}
													...
													{this.state.tableHeading && this.state.tableHeading.actions ?
														<td className="textAlignCenter fixedWidth_new" id="ActionContent">
															<span>
															...
															([key, value1], i) => {
																if ($.type(value1) === 'string') {
																	...
																	if (aN && $.type(aN) === 'string') {
																		...
																		var bN = value1 ? parseInt(value1.replace(this.state.reN, ""), 10) : '';
																		if (bN) {
																			var textAlign = 'textAlignLe fixedWidth';
																		} else {
																			var textAlign = 'textAlignLeft fixedWidth';
																		}
																	}
