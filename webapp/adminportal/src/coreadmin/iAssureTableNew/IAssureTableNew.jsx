...
class IAssureTable extends Component {
	constructor(props) {
		...
		this.state = {
			...
			"showCompanyId": props && props.showCompanyId ? props.showCompanyId : false,
		...
		return (
			...
			<div id="tableComponent" className="col-lg-12 col-sm-12 col-md-12 col-xs-12">
				...
				<div className="col-lg-12 col-sm-12 col-md-12 col-xs-12 NOpadding marginTop8 table-responsive">
				 <table id="entityTable" className="table iAssureITtable-bordered table-striped table-hover fixedTable">
								...
								<tr className="tempTableHeader">
									{this.state.twoLevelHeader.apply === true ?
										this.state.twoLevelHeader.firstHeaderData.map((data, index) => {
											return (
												<th key={index} colSpan={data.mergedColoums} className="umDynamicHeader header-fixed srpadd textAlignCenter">{data.heading}</th>
											);
										})
										:
										null
								...
								<tr className="tempTableHeader">
									{
										...
										<th className="umDynamicHeader  srpadd text-center comp_id" style={{width:"75px"}}>Company ID</th>

										:
										<th className="umDynamicHeader srpadd text-center"style={{height:"40px"}}>Sr. No.</th>
									...
									{this.state.tableHeading ?
										Object.entries(this.state.tableHeading).map(
											([key, value], i) => {
												{/*if (key === 'actions') {
													return (
														<th key={i} className=" umDynamicHeader srpadd text-center new_tableborder heading_font" id="ActionContent">{value}</th>
													);
												} 
												if (key === 'companyName') {
													return (
														<th key={i} className=" umDynamicHeader srpadd text-center new_tableborder heading_font"style={{width:"195px"}}>{value}</th>
													);
												} 
												if (key === 'CompanyID') {
													return (
														<th key={i} className=" umDynamicHeader srpadd onefield_column new_tableborder comp_id">{value}</th>
													);
												} 
												if (key === 'location') {
													return (
														<th key={i} className="text-center"style={{width:"224px"}}>{value}</th>
													);
												} 
												else {
													return (
														<th key={i} className="umDynamicHeader srpadd  textAlignCenter new_tableborder heading_font">{value} <span onClick={this.sort.bind(this)} id={key} className="fa fa-sort tableSort"></span></th>
													);
												}*/}
												if(key === 'actions'){
														return(
									...
									this.state.tableData.map(
										(value, i) => {
											return (
												<tr key={i} className="">
													{ this.state.showCompanyId?
													<td className="textAlignCenter"style={{width:"75px"}}>{value.companyID}</td>
																...
																if (found.length > 0) {
																	if (key !== 'id') {
																		{/*return (<td className={textAlign} key={i}><div className={textAlign} dangerouslySetInnerHTML={{ __html: value1 }}></div></td>);*/}
																			...
																			return(<td className={textAlign} key={j}><div className={textAlign+" col"+(j+1) } dangerouslySetInnerHTML={{ __html:value1}}></div></td>); 						
													...
													{this.state.tableHeading && this.state.tableHeading.actions ?
														<td className="textAlignCenter" style={{width:"160px"}}>
															<span>
				...
				{/*Export To Excel*/}
					<div className="table-responsive" id="HideTable">
						<table className="table iAssureITtable-bordered table-striped table-hover  header-fixed fixedTable" id={"Download-"+this.state.id}>
										...
										Object.entries(this.state.tableHeading).map(
											([key, value], i) => {
												if (key === 'actions') {
													...
													return (
														<th key={i} className="umDynamicHeader header-fixed srpadd textAlignLeft">{value} <span onClick={this.sort.bind(this)} id={key} className="fa fa-sort tableSort"></span></th>
													);
												}
								...
								{this.state.tableData && this.state.tableData.length > 0 ?
									this.state.tableData.map(
										(value, i) => {
											return (
												<tr key={i} className="">
													<td className="textAlignCenter fixedWidth"style={{width:"5%"}}>{this.state.startRange + 1 + i}</td>
													{
														Object.entries(value).map(
															([key, value1], i) => {
																if ($.type(value1) === 'string') {
