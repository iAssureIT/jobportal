...
exports.insertPerson = (req, res, next) => {
    PersonMaster.find(
        {"company_Id":req.body.company_Id,$and: [{"employeeId":req.body.employeeId, 'email':req.body.email}, { 'email':req.body.email }, {"employeeId":req.body.employeeId } ] })
        .exec()
        .then(data=>{
            if(data.length > 0)
            {
            ...
            empPriority: req.body.empPriority,
            type: req.body.type,
            ...
            workLocationDistrict: req.body.workLocationDistrict,
            branchCode: req.body.branchCode,
            badgeNumber: req.body.badgeNumber,
            approvingAuthorityId1: req.body.approvingAuthorityId1 ? (req.body.approvingAuthorityId1).trim() : null,
            approvingAuthorityId2: req.body.approvingAuthorityId2 ? (req.body.approvingAuthorityId2).trim() : null,
            approvingAuthorityId3: req.body.approvingAuthorityId3 ? (req.body.approvingAuthorityId3).trim() : null,
...
exports.postlistPersons = (req,res,next)=>{
    console.log("req body",req.body);
        PersonMaster.findOne({ userId: req.body.id})
...
exports.postOnePersons = (req,res,next)=>{
    console.log("req body",req.body);
        PersonMaster.findOne({ _id: req.body.id})
...
exports.listPersons = (req, res, next) => {
    ...
    PersonMaster.aggregate([
        ...
        { $match: { type: req.body.type , entityType: req.body.entityType} }
    ])
        .sort({ createdAt: -1 })
        .then(data => {
            res.status(200).json(data);
...
exports.listPersonswithusedetails = (req, res, next) => {
    // PersonMaster.find({ type: req.body.type })
    PersonMaster.aggregate([
        {
            $lookup:
...
exports.updatePersonProfileStatus = (req, res, next) => {
    PersonMaster.updateOne(
        { userId: req.body.userId },
        {
            $set: {
    ...
    PersonMaster.updateOne(
        { _id: req.body.personID },
        {
            $set: {
                ...
                'workLocationDistrict':req.body.workLocationDistrict,
                'workLocation': req.body.workLocation,
                'approvingAuthorityId1': req.body.approvingAuthorityId1? (req.body.approvingAuthorityId1).trim():"",
                'approvingAuthorityId2': req.body.approvingAuthorityId2 ? (req.body.approvingAuthorityId2).trim():"",
                'approvingAuthorityId3': req.body.approvingAuthorityId3 ? (req.body.approvingAuthorityId3).trim():"",
...
exports.deletePerson = (req, res, next) => {
      PersonMaster.findOne({ _id: req.params.personID })
        ...
        .then(personDetails => {
            ...
            // console.log("personDetails",personDetails.userId);
            var userId = personDetails.userId;
                PersonMaster.deleteOne({ _id: req.params.personID })
                    .exec()
                    .then(data => {
                        console.log("")
                        if (data.deletedCount === 1) {
                            res.status(200).json({ deleted: true, userId:userId });
                        } else {
...
exports.update_driver_document_satus = (req, res, next) => {
    PersonMaster.updateOne(
         {"Documentarray._id":req.body.document_id},
         {
            "$set": { 
                'Documentarray.$.documentProof.status':req.body.status,
                'Documentarray.$.documentProof.remark':req.body.remark
            }
        })
        .exec()
        .then(person => {
            console.log("person",person);
            if(person.nModified == 1) {
                 res.status(200).json("USER_DOCUMENT_STATUS_UPDATED");
            } else {
                res.status(200).json("USER_DOCUMENT_NOT_UPDATED");
            }
        })
        .catch(err => {
            console.log("err",err);
    ...
    PersonMaster.findOne({ _id: req.body.personID_toberecover })
        ...
        .then(user => {
            if (user) {
                ...
                if (user.status === '' || user.status === undefined) {
                    newstatus = 'Active';
                }if (user.status === 'deleted-Active' || user.status === "deleted-active") {
                    newstatus = 'Active';
                }
                if (user.status === 'deleted-Inactive' || user.status === "deleted-blocked") {
                    newstatus = 'Inactive';
                }
                PersonMaster.updateOne(
                    ...
                    .then(data => {
                        ...
                        if (data.nModified == 1) {
                            PersonMaster.updateOne(
                                ...
                                .then(data => {
                                    // res.status(200).json(data);
                                    res.status(200).json("User Restored");
                                })
                        } else {
                            res.status(200).json("USER_NOT_DELETED")
                        }
    ...
    async function main(){
        for (var key in req.body) {
            ...
            if (key == 'designations' && req.body.designations.length > 0) {
                ...
                if(designationsids && designationsids.length > 1){
                    for(let i=1; i<designationsids.length; i++){
                        selector.designationId.push(ObjectID(designationsids[i]._id));
                    }
                }
            }
...
exports.getDriverListForAllocation = (req, res, next) => {
    ...
    PersonMaster.find({ company_Id: req.body.company_Id, type: 'driver',workLocationDistrict: { $regex : req.body.city, $options: "i"}, status: 'Active' })
        .then(validDrivers => {
            ...
            if(validDrivers && validDrivers.length > 0) {
                for(var i = 0; i < validDrivers.length; i++) {
                    ...
                    if(documentArray.length > 0 && (documentArray[0].documentName === "License" || documentArray[0].documentName === "Driving License" || documentArray[0].documentName === "Driving License Number" || documentArray[0].documentName === "License Number") && new Date(documentArray[0].documentValidTo) <= new Date(desiredToDate)) {
                        validDrivers[i] = {
                            ...validDrivers[i]._doc,
                ...
                BookingMaster.find({
                    ...
                    statusValue: { $nin: ["Cancel By User","Cancel By Vendor","Cancel By Admin","Ready To Bill","Expense Submitted","Reached Garage","End OTP Verified","Reached Drop Location","Trip Completed","Cancelled"] },
                    $or: [
                        {
                            $and: [
                    ...
                    .then(mapping => {
                        if (mapping) {
                            BookingMaster.findOne({ 
                                    allocatedToDriver: req.body.driverID,
                                ...
                                .then(booking => {
                                    if (booking) {
                                        res.status(200).json({message:"Trip allocated to this driver is currently running. You may delete it after the running trip is ended."})
                                    } else {
...
exports.countEmployeesByStatus = (req, res, next) => {
    PersonMaster.find({companyID:req.params.company,status:req.params.status}).count()       
        .exec()
        .then(data=>{
            res.status(200).json({count: data});
        })
...
exports.inactiveemployeeCount = (req, res, next) => {
    PersonMaster.find({companyID:req.params.company,status:{$nin:"Active"}}).count()       
        .exec()
        .then(data=>{
            res.status(200).json({count: data});
        })
...
exports.companyDriverCount = (req, res, next) => {
    PersonMaster.find({ company_Id: req.params.company, type: "driver", status: 'Active' }).count()       
        .exec()
        .then(data=>{
            res.status(200).json({count: data});
        })
