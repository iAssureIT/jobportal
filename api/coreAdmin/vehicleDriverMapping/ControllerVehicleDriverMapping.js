...
exports.insertVehicleDriverMapping = (req,res,next)=>{
   processData();
    async function processData(){
        ...
        if ((data.vehicleID == req.body.vehicleID) && (data.driverID == req.body.driverID) && (data.company_Id == req.body.company_Id) && (data.status == 'Active')) {
            return data;
        }
        })
        if(event.length > 0) {
        ...
        .then(data=>{
            ...
            if(data && data.length > 0){
              var allData = data.map((x, i)=>{
                var driverMiddleName = x.middleName ? x.middleName : ""
                return {
                    "_id"                   : x._id,
                    ...
                    "driverName"            : x.firstName + " " + driverMiddleName + " " + x.lastName,
                }
              })
