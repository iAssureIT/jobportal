...
/*var getFunctionalAreas = async (req,res,next)=>{
    return new Promise(function(resolve,reject){ 
      FunctionalAreaMaster.aggregate([
    {
    $lookup:
        {
...
exports.fetchFunctionalAreaData = (req, res, next)=>{
    FunctionalAreaMaster.find({industry_id:req.params.id})
        .sort({createdAt : -1})
        .exec()
        .then(data=>{
            res.status(200).json(data);
...
exports.searchFunctionalArea = (req, res, next)=>{
    ...
    //FunctionalAreaMaster.find({ model: { $regex : req.params.str ,$options: "i" }  })
        .exec()
        .then(data=>{
            res.status(200).json(data);
        })
...
exports.bulkUploadFunctionalArea = (req, res, next)=>{
    ...
    async function processData(){
        ...
        FunctionalAreaMaster.insertMany(validData)
        .then(data=>{
...
exports.fetch_file = (req,res,next)=>{ 
    FunctionalAreaMaster.find( { _id : "fileName"})
    .exec()
    .then(data=>{
