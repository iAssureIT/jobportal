...
exports.insertCityName = (req,res,next)=>{
    ...
    var cityName = allcityName.filter((data)=>{
        if ( (data.cityTypeId == req.body.dropdownID && data.cityName.trim().toLowerCase() == req.body.fieldValue.trim().toLowerCase()) || (data.cityName.trim().toLowerCase() == req.body.fieldValue.trim().toLowerCase())) {
            return data;
        }
        })
...
exports.getCityNamesList = (req, res, next)=>{
    CityNameMaster.aggregate([
    {
    $lookup:
        {
