...
exports.getVehicleListForAllocation = (req,res,next)=>{
   console.log("req.body",req.body) 
    ...
    VehicleMaster.find(selector)
        .then(validVehicles=>{
            console.log("validVehicles",validVehicles);
                ...
                .then(vehDrivers =>{
                    ...
                    BookingMaster
                        ...
                        .then(bookedVehicleID =>{
                            console.log("bookedVehicleID",bookedVehicleID);
    ...
    VehicleMaster.findOne({_id:req.body.vehicleID})
        ...
        .then(vehicle=>{
            if(vehicle){
                ...
                .then(booking=>{
                    if(booking){
                        res.status(200).json({message:"Trip allocated to this Vehicle is currently running. You may delete it after the running trip is ended."})
                    }else{
...
exports.countCompanyVehicles = (req, res, next)=>{
    VehicleMaster.find({company_Id: req.params.company_Id}).count()
        .exec()
        .then(data=>{
            res.status(200).json({ count : data });
        })
...
exports.update_vehicle_document_status = (req, res, next) => {
    VehicleMaster.updateOne(
         {"vehicleDocument._id":req.body.document_id},
         {
            "$set": { 
                'vehicleDocument.$.status':req.body.status,
                'vehicleDocument.$.remark':req.body.remark
            }
        })
        .exec()
        .then(vehicle => {
...
exports.updateVehicleStatus = (req, res, next) => {
    VehicleMaster.updateOne(
        { _id: req.body.vehicleID },
        {
            $set: {
